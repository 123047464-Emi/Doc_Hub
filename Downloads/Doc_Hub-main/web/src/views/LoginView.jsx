...
export default function LoginView() {
  const { login } = useApp();
  const [username, setUsername] = useState('juez1');
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: '28px' }}>
          ...
          <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginTop: '4px' }}>
            Sistema Integral de Gestión y Control Documental (UPQ)
          </p>
        </div>
            ...
            {USERS.slice(0, 4).map((u) => (
              ...
              >
                ...
                <div style={{ overflow: 'hidden' }}>
                  <div style={{ fontSize: '0.75rem', fontWeight: 700, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{u.name}</div>
                  <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)' }}>{u.role}</div>
                </div>
              </button>
            ))}
          </div>
        ...
        <div style={{ marginTop: '24px', textAlign: 'center', fontSize: '0.75rem', color: 'var(--text-muted)' }}>
          ...
          Acceso seguro encriptado SSL/TLS · Universidad
        </div>
      </div>
    </div>
  );
