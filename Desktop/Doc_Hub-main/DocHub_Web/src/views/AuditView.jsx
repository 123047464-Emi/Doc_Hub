...
export default function AuditView() {
  ...
  const handleExportCSV = () => {
    ...
    csvContent += 'ID Log,Usuario,Accion Realizada,Documento / Folio,Fecha y Hora,Direccion IP,Resultado\n';
    ...
    filteredLogs.forEach(log => {
      const row = [
        ...
        `"${(log.documento || '').replace(/"/g, '""')}"`,
        `"${(log.fecha || '').replace(/"/g, '""')}"`,
        `"${(log.ip || '').replace(/"/g, '""')}"`,
        `"${(log.resultado || '').replace(/"/g, '""')}"`
      ];
      csvContent += row.join(',') + '\n';
    });
    ...
    link.setAttribute('download', `Auditoria_Forense_DocHub_${new Date().toISOString().slice(0, 10)}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);

    showToast(`Bitácora de auditoría forense exportada exitosamente en CSV (${filteredLogs.length} registros)`, 'success');
  };
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
        <div>
          ...
          <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)', marginTop: '2px' }}>
            Registro inalterable de accesos, firmas e interacciones en la plataforma
          </p>
        </div>

        <button className="btn btn-primary" onClick={handleExportCSV}>
          <Download size={16} /> Exportar Bitácora Completa (CSV / Excel)
        </button>
      </div>
