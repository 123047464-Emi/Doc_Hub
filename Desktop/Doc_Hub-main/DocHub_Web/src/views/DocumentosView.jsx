    ...
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '24px', position: 'relative' }}>
      ...
      <div style={{
        ...
        <div>
          ...
          {user?.role === 'Juez' ? (
            <p style={{ fontSize: '0.82rem', color: 'var(--accent)', marginTop: '2px', fontWeight: 600 }}>
              Mostrando únicamente los expedientes asignados a: {user.name} ({user.cargo})
            </p>
          ) : (
            <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)', marginTop: '2px' }}>
              Consola del Administrador · Visualización y supervisión global de expedientes del sistema
            </p>
          )}
        </div>
      ...
      <div className="card-glass" style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
          ...
          >
            <AlertCircle size={14} /> Importancia Alta
          </button>
        </div>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.875rem' }}>
            <thead>
              <tr style={{ backgroundColor: 'var(--bg-app)', borderBottom: '1px solid var(--border-color)', color: 'var(--text-secondary)', textTransform: 'uppercase', fontSize: '0.72rem', letterSpacing: '0.05em' }}>
                <th style={{ padding: '14px 20px', width: '110px' }}>Importancia</th>
                  ...
                  >
                    ...
                    <td style={{ padding: '14px 20px' }}>
                      <button
                        ...
                        title="Haga clic para alternar nivel de importancia"
                      >
                        <span className={`badge ${doc.favorito ? 'badge-danger' : 'badge-info'}`} style={{ fontSize: '0.72rem' }}>
                          {doc.favorito ? 'Alta' : 'Normal'}
                        </span>
                      </button>
                    </td>
      ...
      {/* Floating Action Button (FAB) for Drag & Drop Upload (Juez Only) */}
      {user?.role === 'Juez' && (
        <button
