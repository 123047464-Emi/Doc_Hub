...
export default function SettingsView() {
  ...
  const handleSaveProfile = (e) => {
    ...
    if (name.trim()) {
      user.name = name.trim();
      showToast(`Nombre actualizado a "${name}" correctamente`, 'success');
    }
  };
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div>
        <h2 style={{ fontSize: '1.4rem', fontWeight: 800 }}>Configuración de la Cuenta</h2>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '240px 1fr', gap: '24px' }}>
        ...
        <div className="card-glass" style={{ padding: '12px', height: 'fit-content', display: 'flex', flexDirection: 'column', gap: '4px' }}>
          {[
            ...
            { id: 'firma', label: 'Certificado y Credenciales', icon: Key },
          ].map(item => {
        ...
        <div className="card-glass" style={{ padding: '28px' }}>
          {activeTab === 'perfil' && (
            <form onSubmit={handleSaveProfile} style={{ display: 'flex', flexDirection: 'column', gap: '18px', maxWidth: '540px' }}>
              ...
              <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', backgroundColor: 'var(--bg-app)', padding: '12px 16px', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <Lock size={16} color="var(--accent)" style={{ flexShrink: 0 }} />
                <span>El correo electrónico institucional no se puede modificar por políticas de seguridad centralizada.</span>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                ...
                />
                <div>
                  <div style={{ fontSize: '1rem', fontWeight: 700 }}>{name}</div>
                  <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{cargo} · {user.role}</div>
                </div>
              </div>

              <div>
                <label className="input-label">Nombre Completo del Titular</label>
                <input
                  type="text"
              ...
              <div>
                <label className="input-label">Correo Electrónico Institucional (No modificable)</label>
                <input type="email" className="input-field" value={email} disabled style={{ cursor: 'not-allowed', backgroundColor: 'var(--bg-app)', color: 'var(--text-secondary)' }} />
              </div>

              <div>
                <label className="input-label">Cargo Judicial / Rol Asignado (Modificable sólo por Administrador)</label>
                <input
                  type="text"
                  className="input-field"
                  value={`${user.role} · ${cargo}`}
                  disabled
